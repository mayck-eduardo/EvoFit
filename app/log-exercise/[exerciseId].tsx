...
export default function LogExerciseScreen() {
  ...
  const { exerciseId, exerciseName, routineId, routineName, exerciseSets } = params;
  ...
  const handleSaveLog = async () => {
    ...
    try {
      ...
      await addDoc(
        collection(db, 'artifacts', appId, 'users', user.uid, 'recentActivity'),
        {
        ...
        >
          ...
          <View style={[styles.infoCard, { backgroundColor: colors.card, borderColor: colors.cardBorder }]}>
            <View style={styles.infoRow}>
              <View>
                <Text style={[styles.infoLabel, { color: colors.textSecondary }]}>Séries Programadas</Text>
                <Text style={[styles.infoValue, { color: colors.text }]}>{setsProgrammed || '-'}</Text>
              </View>
              <View style={styles.setCounter}>
                <Text style={[styles.setCounterText, { color: colors.success }]}>
                  {setsCount}
                  <Text style={[styles.setCounterOf, { color: colors.textMuted }]}>/{(setsProgrammed.match(/\d+/g) || []).join('')}</Text>
                </Text>
                <Text style={[styles.setCounterLabel, { color: colors.textSecondary }]}>feitas hoje</Text>
              </View>
            </View>
            <Pressable
              onPress={() =>
              ...
              style={[styles.chartLink, { borderTopColor: colors.cardBorder }]}
            >
              <FontAwesome name="line-chart" size={16} color={colors.primary} />
              <Text style={[styles.chartLinkText, { color: colors.primary }]}>Ver evolução</Text>
            </Pressable>
          </View>

          {/* Quick Input */}
          <View style={[styles.inputSection, { backgroundColor: colors.surfaceAlt, borderColor: colors.cardBorder }]}>
            <Text style={[styles.sectionTitle, { color: colors.text }]}>Nova Série</Text>

            <View style={styles.quickInputs}>
              <View style={styles.quickInputContainer}>
                <Text style={[styles.quickInputLabel, { color: colors.textSecondary }]}>Peso ({weightUnit})</Text>
                <TextInput
                  style={[styles.quickInput, { backgroundColor: colors.inputBg, color: colors.text, borderColor: 'transparent' }, activeField === 'weight' && { borderColor: colors.primary }]}
                  placeholder="0"
                ...
                <View style={styles.quickAdjust}>
                  <Pressable
                    style={[styles.adjustBtn, { backgroundColor: colors.inputBg }]}
                    onPress={() => {
                  ...
                  >
                    <Text style={[styles.adjustBtnText, { color: colors.text }]}>-2.5</Text>
                  </Pressable>
                  <Pressable
                    style={[styles.adjustBtn, { backgroundColor: colors.inputBg }]}
                    onPress={() => {
              ...
              <View style={styles.quickInputContainer}>
                <Text style={[styles.quickInputLabel, { color: colors.textSecondary }]}>Reps</Text>
                <TextInput
                  style={[styles.quickInput, { backgroundColor: colors.inputBg, color: colors.text, borderColor: 'transparent' }, activeField === 'reps' && { borderColor: colors.primary }]}
                  placeholder="0"
                  placeholderTextColor={colors.textMuted}
                  value={reps}
                ...
                <View style={styles.quickAdjust}>
                  ...
                  >
                    <Text style={[styles.adjustBtnText, { color: colors.text }]}>-1</Text>
                  </Pressable>
                  <Pressable
                    style={[styles.adjustBtn, { backgroundColor: colors.inputBg }]}
                    onPress={() => {
            ...
            {weight && reps && calculateEpley1RM(parseFloat(weight), parseInt(reps)) > 0 && (
              <View style={[styles.oneRMPreview, { backgroundColor: colors.inputBg }]}>
                <Text style={[styles.oneRMLabel, { color: colors.textSecondary }]}>Est. 1RM</Text>
            ...
            <Pressable onPress={() => setShowNoteInput(!showNoteInput)} style={styles.noteToggle}>
              ...
              />
              <Text style={[styles.noteToggleText, { color: colors.textSecondary }]}>
            ...
            {/* Save Button */}
            <TouchableOpacity
              style={[styles.saveButton, { backgroundColor: colors.primary }, logLoading && { backgroundColor: colors.textMuted }]}
            ...
            >
              {logLoading ? (
                ...
                <>
                  ...
                  <Text style={[styles.saveButtonText, { color: '#FFF' }]}>Salvar Série</Text>
                </>
              )}
            </TouchableOpacity>
          </View>
          ...
          {todayLogs.length > 0 && (
            <View style={styles.logsSection}>
              ...
              {todayLogs.map((item, index) => {
                ...
                return (
                  <View key={item.id} style={[styles.logCard, { backgroundColor: colors.card, borderColor: colors.cardBorder }]}>
                    <View style={styles.logHeader}>
                      <View style={[styles.logSetBadge, { backgroundColor: colors.primary }]}>
                        <Text style={[styles.logSetBadgeText, { color: '#FFF' }]}>#{todayLogs.length - index}</Text>
                      </View>
                      <Text style={[styles.logText, { color: colors.text }]}>
                        {item.weight} {weightUnit} × {item.reps}
                      </Text>
                      {estimated1RM > 0 && (
                        <Text style={[styles.log1RM, { color: colors.primary }]}>
                          1RM: {estimated1RM}{weightUnit}
                        </Text>
                      )}
                      <Text style={[styles.logTime, { color: colors.textMuted }]}>{formatLogTime(item.createdAt)}</Text>
                      <Pressable onPress={() => handleDeleteLog(item.id)} style={styles.deleteBtn}>
                        <FontAwesome name="trash-o" size={18} color={colors.textMuted} />
                      </Pressable>
                    </View>
                    {item.note && (
                      <View style={[styles.logNote, { borderTopColor: colors.cardBorder }]}>
                        <FontAwesome name="comment" size={12} color={colors.textMuted} style={{ marginRight: 6 }} />
                        <Text style={[styles.logNoteText, { color: colors.textSecondary }]}>{item.note}</Text>
                      </View>
                    )}
                  </View>
                );
          ...
          {groupOtherLogs().map((group) => (
            <View key={group.date} style={styles.logsSection}>
              <Text style={[styles.sectionTitle, { color: colors.text }]}>{group.date}</Text>
                ...
                return (
                  <View key={item.id} style={[styles.logCard, { backgroundColor: colors.card, borderColor: colors.cardBorder }]}>
                    <View style={styles.logHeader}>
                      ...
                      {estimated1RM > 0 && (
                        <Text style={[styles.log1RM, { color: colors.primary }]}>1RM: {estimated1RM}</Text>
                      )}
                      <Text style={[styles.logTime, { color: colors.textMuted }]}>{formatLogTime(item.createdAt)}</Text>
                      <Pressable onPress={() => handleDeleteLog(item.id)} style={styles.deleteBtn}>
                        <FontAwesome name="trash-o" size={18} color={colors.textMuted} />
...
const styles = StyleSheet.create({
  ...
  setCounterText: { fontSize: 28, fontWeight: '700' },
  setCounterOf: { fontSize: 20 },
  setCounterLabel: { fontSize: 12 },
  chartLink: {
    flexDirection: 'row',
    alignItems: 'center',
    marginTop: 12,
  ...
  chartLinkText: { fontSize: 14, fontWeight: '600' },
  inputSection: {
    marginHorizontal: 16,
    borderRadius: 16,
    padding: 16,
  ...
  quickInputLabel: { fontSize: 13, marginBottom: 6 },
  quickInput: {
    padding: 14,
    borderRadius: 12,
    fontSize: 20,
  ...
  adjustBtnText: { fontSize: 14, fontWeight: '600' },
  oneRMPreview: {
    flexDirection: 'row',
  ...
  oneRMLabel: { fontSize: 14 },
  oneRMValue: { fontSize: 18, fontWeight: '700' },
  noteToggle: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 8,
  ...
  logSetBadgeText: { fontSize: 12, fontWeight: '700' },
  logText: { fontSize: 16, fontWeight: '600', flex: 1 },
  log1RM: { fontSize: 13, fontWeight: '600' },
  logTime: { fontSize: 12 },
  deleteBtn: { padding: 4 },
  logNote: {
    flexDirection: 'row',
  ...
  logNoteText: { fontSize: 13, flex: 1, fontStyle: 'italic' },
});
